...
namespace WindowsService2
{
    public partial class Service1 : ServiceBase
    {
            ...
            if (clientSocket != null)
            {
                ...
                for (; ; )
                {
                    try
                    {
                        ...
                        if (ret == 0)
                        {
                            if (type >= 0 && addr >= 0)       //客户端断开
                            {
                                foreach (Connection c in connection)
                                {
                                    ...
                                    {
                                        connection.Remove(c);
                                        if (File.Exists(defaultpath + "\\" + "类型 " + c.type + " 地址 " + c.addr + "buffer" + ".txt"))
                                            File.Delete(defaultpath + "\\" + "类型 " + c.type + " 地址 " + c.addr + "buffer" + ".txt");
                                        break;
                                    }
                                }
                                string con = "";
                        ...
                        {
                            if (result[0] == end)
                                TS = rx_handler(rx_buf, length, myClientSocket);
                            step = 1;
                            length = 11;
                            continue;
                        }
        ...
        private string rx_handler(byte[] rx_buf, int length, Socket myClientSocket)
        {
